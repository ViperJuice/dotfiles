...
# Subagents directory (assembled from shared/agents/ + symlinked from claude-config/agents/)
mkdir -p ~/.claude/agents
# Clear stale assembled agents and symlinks
...
# Claude-specific agents (symlinked directly)
...
# OpenCode: tool-specific agents (symlinked from opencode-config/agents/)
mkdir -p ~/.config/opencode/agents
for entry in ~/.config/opencode/agents/*; do
    [ -L "$entry" ] && rm -f "$entry"
done
for agent in "$DOTFILES_DIR/opencode-config/agents/"*.md; do
    [ -f "$agent" ] && ln -sf "$agent" ~/.config/opencode/agents/
done
# Remove legacy singular agent/ dir (migrated to plural agents/)
[ -d ~/.config/opencode/agent ] && rm -rf ~/.config/opencode/agent
...
# =============================================================================
# Antigravity IDE Configuration (shares ~/.gemini/ with Gemini CLI)
# =============================================================================

if command -v antigravity &>/dev/null || [ -d ~/.gemini/antigravity ]; then
    mkdir -p ~/.gemini/antigravity/skills

    # Symlink shared skills (Antigravity uses same SKILL.md format as Claude Code)
...
MCPEOF
    ...
    echo "Configured Antigravity IDE (skills + MCP gateway)"
...
if command -v pi &>/dev/null || [ -d ~/.pi/agent ]; then
...
for tool in "${CONFIGURED[@]}"; do
    case "$tool" in
        "Gemini CLI"|"Cursor IDE"|"Antigravity IDE"|"PI agent"|"OpenCode MCP")
            echo "  • $tool"
            ;;
    esac
done
